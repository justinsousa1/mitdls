...
terraform {
  backend "s3" {
    ...
    key = "terraform-states/staging/staging-wordpress-site.tfstate"
...
module "cloudwatch_alarms" {
  ...
  context = local.context
  alert_sns_topic_arn = "alert-topic-arn"
  load_balancer_arn_suffix = module.wordpress_site.lb_arn_suffix
  target_group_arn_suffix = module.wordpress_site.https_tg_arn_suffix
}
